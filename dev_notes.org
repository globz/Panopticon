...
+ Maybe persist turn, sq_turn, compound_turn settings for game + (no branch) - (Implemented 2025-02-04)
+ Persist previousBranch (new column) unto the game + (no branch) setting
...
+ Force SwitchBranch & Git.Checkout add a checkoutOptions and use the following modifier => CheckoutModifiers.Force
+ Fix missing timeline node when Exiting replay mode (Implemented 2025-02-05)
+ Fix Auto UI text in settings when radio button is disabled (Implemented 2025-02-05)
